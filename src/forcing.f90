...
! For helical forcing in spherical polar coordinate system
  ...
  logical :: lisotropize_SR=.false., SR_alpha_shift=.false., SR_ell_shift=.false., lfastSR=.false.
! allocated only if we have lfastCK=T
  real,allocatable,dimension(:,:,:) :: Zpsi_list
  real,allocatable,dimension(:,:,:) :: RYlm_list,IYlm_list
! allocated only if we have lfastSR=T
  real,allocatable,dimension(:,:,:,:) :: RYlm_rot,IYlm_rot
  integer :: nlist_ck=25, ell_shift=0, helsign=0., ncol=7, n_axisrot_angles=10
  real :: fpre = 1.0,ck_equator_gap=0.,ck_gap_step=0.,alpha_shift=5.
...
!
  namelist /special_run_pars/ &
       ...
       lfastCK,lisotropize_SR,fpre,helsign,nlist_ck,lwrite_psi,&
...
!                      radial direction that consists of Bessel functions in the 
!                      radial and randomized spherical Harmonics around the x-axis
...
!
      elseif (iforce_profile=='1+(z-z0)^n') then
        ...
        elseif (height_ff<0) then
          ...
          zstar=impossible
          call fatal_error('initialize_forcing',"needs height_ff/=0 for iforce_profile='1+(z-z0)^n'")
        endif
...
!
      elseif (iforce_profile=='1+(z-z0)^n_prof') then
        ...
        elseif (height_ff<0) then
          ...
          zstar=impossible
          call fatal_error('initialize_forcing',"needs height_ff/=0 for iforce_profile='1+(z-z0)^n_prof'")
        endif
...
!
      elseif (iforce_profile=='exp(z/H)') then
        ...
        profz_hel=1.
!
      elseif (iforce_profile=='exp(x/H)') then
        profz_ampl=1.; profz_hel=1.
        profy_ampl=1.; profy_hel=1.
        profx_ampl=exp(x(l1:l2)/width_ff)
        profx_hel=1.
!
...
!
        ...
        call fatal_error('initialize_forcing','no such iforce_profile: '//trim(iforce_profile))
      endif
!
...
!
          ...
          if (lk_dot_dat_exists) then
            ...
            call inevitably_fatal_error ('initialize_forcing','you must provide a k_double.dat file')
          endif
        endif
!
      elseif (iforce=='spherical_radial') then  
!
        if (.not. lspherical_coords) call fatal_error('initialize_forcing', &
                                     'spherical_radial forcing works only in spherical coordinates!')
!
! Read the list of values for emm, ell and alpha from file "alpha_in.dat". 
! The number of given alpha for each lm pair should be ncol-2. 
!
        open(unit=76,file="alpha_in.dat",status="old")
        read(76,*) ckno,rmin,rmax
        if (ckno/=nlist_ck) then
          call warning("initialize_forcing", &
...
!
        if (lfastSR.and.lisotropize_SR) then
          if (.not.allocated(RYlm_rot)) then
            allocate(RYlm_rot(my,mz,nlist_ck,n_axisrot_angles))
            allocate(IYlm_rot(my,mz,nlist_ck,n_axisrot_angles))
          endif
            ...
            do iangle=1,n_axisrot_angles
              do n=1,mz
                do m=1,my
                  ...
                  sthphase=sin(ang_intv*iangle); cthphase=cos(ang_intv*iangle)
                  ...
                  RYlm_rot(m,n,ilist,iangle)=RYlm
                  IYlm_rot(m,n,ilist,iangle)=IYlm
                enddo
              enddo
            enddo
          enddo
...
!
        open(unit=76,file="alpha_in.dat",status="old")
        read(76,*) ckno,rmin,rmax
        if (ckno/=nlist_ck) call fatal_error("initialize_forcing", &
          "Number of entries in alpha_in.dat "//trim(itoa(ckno))//" unequal nlist_ck="//trim(itoa(nlist_ck)))
        ...
        if (lfastCK) then
          ...
          do ilist=1,nlist_ck
            ...
            do aindex=1,3
              ...
              call sp_bessely_l(anum,Legendrel,Balpha*xyz0(1))
              call sp_besselj_l(adenom,Legendrel,Balpha*xyz0(1))
              a_ell = -anum/adenom
              do l=1,mx
                alphar=Balpha*x(l)
...
!
        if (iforcing_cont(i)=='ABC' .or. iforcing_cont(i)=='Straining') then
        ...
        elseif (iforcing_cont(i)=='(sinz,cosz,0)') then
          sinz(:,i)=sin(kf_fcont(i)*z)
          cosz(:,i)=cos(kf_fcont(i)*z)
        elseif (iforcing_cont(i)=='(0,cosx*cosz,0)' .or. &
                iforcing_cont(i)=='(0,cosx*cosz,0)_Lor') then
...
!
      if ( (t>tforce_stop) .or. (t<tforce_start) ) then
        ...
        if ( iforce/='zero' .and. lfirstforce .and. lroot ) lfirstforce=.false.
...
!
        ...
        case ('spherical_radial');call forcing_spherical_radial(f)    
...
!
      if ( (t>tforce_stop2) .or. (t<tforce_start2) ) then
        ...
        if ( (iforce2/='zero') .and. lfirstforce2 .and. lroot) print*,'addforce: addforce2 started'
        lfirstforce2=.false.
!
        select case (iforce2)
...
!
      if (force_strength/=0.) then
        ...
        if (laniso_forcing_old) then
          fda = 1. + (force_strength/force)*(kk*force_direction/(k*fd))**2 * force_direction/fd
        else
          fda = sqrt(1. + (force_strength/force)*(dot_product(kk,force_direction)/(k*fd))**2)
        endif
      else
        fda = 1.
      endif
...
!
      ...
      select case (iforcing_zsym)
        ...
        case default
          call fatal_error('fxyz_coefs_hel','no such iforcing_zsym: '//trim(itoa(iforcing_zsym)))
      endselect
!
...
!
      if (rcyl_ff/=0.) then
!       if (lroot) print*,'fxyz_coefs_hel: applying sgn(z)*xi(r) profile'
        !
...
!  unaffected when changing k1_ff).
!
        ffnorm=sqrt(1.+relhel**2)*k*sqrt(k2-kde**2)/sqrt(kav*cs0**3)*(k/kav)**slope_ff
        if (ip<=9) then
...
!
        if (force_strength/=0.) then
          ...
          fd=sqrt(fd2)
          do j=1,3
            fda(:,j) = 1. + (force_strength/force)*(kk(j)*force_direction(j)/(k*fd))**2 &
...
!
                forcing_rhs2(:,j)=rho1*profx_ampl*profy_ampl(m)*profz_ampl(n)*force_ampl &
                                  *real(cmplx(0.,coef3(j))*fx(l1:l2)*fy(m)*fz(n))*fda(:,j)
!
                if (ifff/=0) then
!
                  jf=j+ifff-1
...
!
                ...
                forcing_rhs(:,j)= rho1*profx_ampl*profy_ampl(m)*profz_ampl(n) &
                                 *real(cmplx(coef1(j),profy_hel(m)*coef2(j))*fx(l1:l2)*fy(m)*fz(n))
                if (lhelical_test) then
                  f(l1:l2,m,n,jf)=forcing_rhs(:,j)
                else
                  f(l1:l2,m,n,jf)=f(l1:l2,m,n,jf)+forcing_rhs(:,j)
...
!
! Calculate the "potential" for the helical forcing according to Chandrasekhar & Kendall.
! Construct Z_psi(r).
!
      ...
      if (lfastCK) then
        ...
        call sp_besselj_l(adenom,Legendrel,Balpha*xyz0(1))
        a_ell = -anum/adenom
!        write(*,*) 'dhruba:',anum,adenom,Legendrel,Bessel_alpha,xyz0(1)
        do l=1,mx
          alphar=Balpha*x(l)
          call sp_besselj_l(jlm_ff,Legendrel,alphar)
          call sp_bessely_l(ylm_ff,Legendrel,alphar)
...
!***********************************************************************
    subroutine forcing_spherical_radial(f)
!
...
!
      if (lhelical_test) then
        if (icklist==nlist_ck) &
          call fatal_error("forcing_spherical_radial","SR testing: no more values in list")
        icklist=icklist+1
        lmindex=icklist
      else
        call random_number_wrapper(rindex,CHANNEL=channel_force)
...
!
      ...
      Balpha = cklist(lmindex,3+aindex)
!
      if (SR_alpha_shift) then
        call random_number_wrapper(ralpha2,CHANNEL=channel_force)
        alphap=alpha_shift*2*ralpha2-alpha_shift
        Balpha=Balpha+alphap
      endif
! 
      if (SR_ell_shift) then
        call random_number_wrapper(ellSR,CHANNEL=channel_force)
        ell_rand=nint(ellSR*2*ell_shift)-ell_shift
        Legendrel=Legendrel+ell_rand 
      endif
!
! Calculate the "potential" for the non-helical forcing according to Chandrasekhar & Kendall.
! Construct Z_psi(r)
!
      call sp_bessely_l(anum,Legendrel,Balpha*xyz0(1))
      call sp_besselj_l(adenom,Legendrel,Balpha*xyz0(1))
...
!
      ...
      if (lisotropize_SR) then

        call random_number_wrapper(rphase1,CHANNEL=channel_force)
        rphase1=rphase1*2*pi

        if (lfastSR) then
          call random_number_wrapper(gindex,CHANNEL=channel_force)
          raindex=nint(gindex*(n_axisrot_angles-1))+1
          crphase1=cos(rphase1)
          srphase1=sin(rphase1)
          do n=1,mz
            do m=1,my
              psilm = RYlm_rot(m,n,lmindex,raindex)*crphase1 - &
                      IYlm_rot(m,n,lmindex,raindex)*srphase1
              psif(:,m,n) = Z_psi*psilm
            enddo
          enddo
        else
!
! Randomize axis of spherical harmonic by rotation about the x direction.
!
...
!
        ...
        do n=n1,n2
          do m=m1,m2
            ...
            call gij_psi_etc(psif,ee,psi,psi_ij,Tij)
            call curl_mn(Tij,capitalS,capitalT)
            capitalS(:,1) = (1./Balpha)*capitalS(:,1)
            if (r_ff /= 0.) capitalS(:,1)=profx_ampl*capitalS(:,1)
            if (lhelical_test) then
              if (lwrite_psi) then
                f(l1:l2,m,n,iux) = psif(l1:l2,m,n)
              else
                f(l1:l2,m,n,iux) = fpre*fnorm*capitalS(:,1)
              endif
            else
              fmaxloc=max(fmaxloc,maxval(abs(capitalS(:,1))))
              f(l1:l2,m,n,iff_aux) = fpre*capitalS(:,1)
            endif
          enddo
        enddo
  
        if (.not.lhelical_test) then
          call mpiallreduce_max(fmaxloc,fmax)
!
! Stochastic euler scheme of integration [sqrt(dt) is already included in fnorm].
!
          f(l1:l2,m1:m2,n1:n2,iux) = (1./fmax)*f(l1:l2,m1:m2,n1:n2,iff_aux)
        endif
      endif

    endsubroutine forcing_spherical_radial
!***********************************************************************
...
!
      if ( (dtforce_duration<0.0) .or. (t-(tsforce-dtforce))<=dtforce_duration ) then
!
!  Normalize ff; since we don't know dt yet, we finalize this
!  within timestep where dt is determined and broadcast.
!
...
!
        if (lroot .and. lwrite_gausspot_to_file) then
          open(1,file=trim(datadir)//'/hillrain_forcing.dat',status='unknown',position='append')
          write(1,'(4f14.7)') t, location
          close (1)
        endif
!
...
!
      ...
      if (lforce) call blob(force,f,iss,radius_ff,location(1),location(2),location(3))
!
...
!
      ...
      do n=n1,n2
        do m=m1,m2
          ...
          do l=l1,l2
            dist3 = sqrt( (R0_tidal*cos(omega_tidal*t)*cos(phi_tidal)-x(l))**2 + &
                          (R0_tidal*sin(omega_tidal*t)               -y(m))**2 + &
                          (R0_tidal*cos(omega_tidal*t)*sin(phi_tidal)-z(n))**2  )**3
            forcing_rhs(l-l1+1,1)=rho1(l)*fact*(R0_tidal*cos(omega_tidal*t)*cos(phi_tidal)-x(l))/dist3
            forcing_rhs(l-l1+1,2)=rho1(l)*fact*(R0_tidal*sin(omega_tidal*t)-y(m))/dist3
            forcing_rhs(l-l1+1,3)=rho1(l)*fact*(R0_tidal*cos(omega_tidal*t)*sin(phi_tidal)-z(n))/dist3
          enddo
          f(l1:l2,m,n,iffx:iffz)=f(l1:l2,m,n,iffx:iffz)+forcing_rhs
          if (lout) then
            if (idiag_rufm/=0) then
...
!
      if (r_ff/=0.) then
        if (lroot) print*,'hel_vec: applying sgn(z)*xi(r) profile'
        !
        ! only z-dependent part can be done here; radial stuff needs to go
        ! into the loop
        !
...
!
      ...
      do ii=1,nk_GP
        ...
        select case (sp)
        case('xyz')
          sinxt(:,i) = sinxt(:,i) + k*( knorm**(-beta_GP) ) * sin( k*(x-xi +cos(omega*t) ) )
          cosxt(:,i) = cosxt(:,i) + k*( knorm**(-beta_GP) ) * cos( k*(x-xi +cos(omega*t) ) )
          sinyt(:,i) = sinyt(:,i) + k*( knorm**(-beta_GP) ) * sin( k*(y-eta+sin(omega*t) ) )
          cosyt(:,i) = sinyt(:,i) + k*( knorm**(-beta_GP) ) * cos( k*(y-eta+sin(omega*t) ) )
        case('yzx')
          sinzt(:,i) = sinzt(:,i) + k*( knorm**(-beta_GP) ) * sin( k*(z-xi +cos(omega*t) ) )
          coszt(:,i) = coszt(:,i) + k*( knorm**(-beta_GP) ) * cos( k*(z-xi +cos(omega*t) ) )
          sinxt(:,i) = sinxt(:,i) + k*( knorm**(-beta_GP) ) * sin( k*(x-eta+sin(omega*t) ) )
          cosxt(:,i) = sinxt(:,i) + k*( knorm**(-beta_GP) ) * cos( k*(x-eta+sin(omega*t) ) )
        case default
...
!
        case('StrainingExcact')
          call fatal_error("forcing_cont","'StrainingExcact' not tested yet")
...
!
        case ('(0,Aycont_z,0)')
          force(:,1)=0.
          force(:,2)=-ampl_ff(i)*((Bconst*(z(n)-xyz0(3)))+(0.5*Bslope*((z(n)-xyz0(3))**2.)))
          force(:,3)=0.
!
!  f=(sinz,cosz,0)
!
...
!
      case ('KS')
        ...
        do modeN=1,KS_modes  ! sum over KS_modes modes
          ...
          force(:,1) = force(:,1) + cos_kdotxwt*KS_A(1,modeN) + sin_kdotxwt*KS_B(1,modeN)
          force(:,2) = force(:,2) + cos_kdotxwt*KS_A(2,modeN) + sin_kdotxwt*KS_B(2,modeN)
          force(:,3) = force(:,3) + cos_kdotxwt*KS_A(3,modeN) + sin_kdotxwt*KS_B(3,modeN)
        enddo
...
!
      if (ip<=6.and.lroot .and. (tsforce>=0.)) print *,'output_persistent_forcing: ',location,tsforce
!
!  write details
!
